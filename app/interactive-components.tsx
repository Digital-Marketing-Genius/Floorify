...
export function ShowroomCatalog() {
  ...
  return <>
    ...
    {selected && <div className="product-modal" role="dialog" aria-modal="true" aria-label={selected.name} onClick={() => setSelected(null)}><div className="product-modal-card" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)}>×</button><div className="modal-images"><img src={selected.image} alt={selected.name}/>{selected.images?.length>1&&<div>{selected.images.map(img=><figure key={img.url}><img src={img.url} alt={`${selected.name} — ${img.label}`} loading="lazy"/><figcaption>{img.label}</figcaption></figure>)}</div>}</div><div><p className="eyebrow">{labels[selected.category]}</p><h2>{selected.name}</h2><p>{selected.description}</p><dl><div><dt>Collection</dt><dd>{selected.collection}</dd></div><div><dt>Type</dt><dd>{selected.type}</dd></div><div><dt>Width</dt><dd>{selected.width}</dd></div><div><dt>Finish</dt><dd>{selected.finish}</dd></div><div><dt>Warranty</dt><dd>{selected.warranty}</dd></div></dl><a className="button button-dark" href={`/#estimate`}>Request a quote →</a></div></div></div>}
  </>;
}
